...
def inference(test_loader, model, cfg, result_path):
    ...
    with torch.no_grad():
        for i_iter, batch in enumerate(test_loader):
            ...
            equi_img = input_list[0].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_B = input_list[1].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_D = input_list[2].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_F = input_list[3].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_L = input_list[4].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_R = input_list[5].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
            cubemap_T = input_list[6].type(torch.FloatTensor).to(torch.device('cpu', cfg['device_id']))
...
def main():
    ...
    if config['use_gpu']:
        model = model.to(torch.device('cpu', config['device_id']))
